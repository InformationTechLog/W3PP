...
contract Ballot is Team {
    ...
    function changeState(Phase x) onlyLeader public {
        state = x;
    }

    function setAddrName(address _addr, string memory _name) public validPhase(Phase.Regs) onlyLeader { // 주소와 이름을 각각 주소배열, 이름배열에 넣어주는 과정.
    ...
    function setUser() public validPhase(Phase.Regs) onlyLeader {  // 주소와 이름배열에 들어갔던 내용이 struct user에 들어가고 mapping 형식의 Users로 관리된다.
    ...
    function setPoll(string memory _title) public validPhase(Phase.Regs) onlyLeader { // 투표 설정하기 => poll(몇번째투표인지, 투표이름, 그 투표에 걸린 토큰)
    ...
    function setTeam(string memory _teamName) public validPhase(Phase.Regs) onlyLeader { // 팀 설정(수업명, 팀번호, 팀이름, 팀멤버주소, 안건개수, 토큰총발행량)
    ...
    function getTotalTokenSupply() public view returns(uint) {  // 토큰 총 공급량
        return MT.getTotalSupply();
    }

    function getUserToken(address _addr) public view returns(uint) {  // 그 사람이 토큰을 몇 개 가지고 있는지
        return Users[_addr].token;
    }

    mapping(address => uint) voting; // 해당 지갑주소인 사람이 몇 표를 얻었는지

    function vote(uint _point, address _addr) public validPhase(Phase.Vote) returns(uint) { // _addr : 누구에게, _point : 몇개를 투표할지
        voting[_addr] += _point;
    ...
    function ranking() public validPhase(Phase.Done) returns(address[] memory) { // 투표 많이 받은 순으로 저장된 배열 출력
    ...
    function getToken() public validPhase(Phase.Done) onlyLeader { // 1등한테 안건에 달린 토큰을 지급하고 받은 투표수 초기화, 각 사람마다 투표에 쓸 수 있는 포인트 100으로 초기화 + 팀장은 20포인트 더 줌.
